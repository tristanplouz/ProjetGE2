...
on notera que hard timing: surtout pour les cages tournantes.
	...
	\chapter{Contrôle}
	Le système est controlé par deux microcontrôleurs un \texttt{PIC16F1619} ainsi qu'un \texttt{dsPIC10F2030}.
...
\end{table}
		...
		\section{Fonctionnement}
			...
			\subsection{Sécurité}
				...
				\subsubsection{Perte de connection}
	...
	La conception mécanique est un point important dans la réalisation de notre aéroglisseur. La structure doit en effet supporter tous les composants cités ci-dessus (batterie, moteur, carte, ...). Cela doit aussi correspondre au cahier des charges. Le cahier des charges spécifie des dimensions de 25cm de largeur, 30cm de hauteur et 35cm de longueur, le matériau doit être le carton plume. De plus l'hélice doit être protégée et un support de caméra sportive doit pouvoir être fixé. 
		...
		Toutes les modifications que nous avons apporté au modèle ont été réalisé sur le logicel de \textit{Conception Assistée par Ordinateur} \href{https://freecadweb.org/}{FreeCAD} en re modélisant les pièces du modèle originel et en modélisant de nouvelles.
...
%A METTRE A JOUR
			...
			\subsection{Sécurité} Une cordelette pend derière l'aéroglisseur. En tirant sur cette cordelette, on débranche un jumper, qui désactive le moteur de propulsion. Cette fonctionnalité est \hyperref[secu]{définie ici}, dans la partie contrôle. De plus, un grillage est prévu autour de l'hélice pour en assuré la protection.
			\begin{figure}[h]
					\begin{center}
