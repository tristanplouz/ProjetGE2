...
\end{table}
	...
	\end{figure}
		...
		\section{Réalisation}\label{real}
			...
			La réalisation a été faite avec les \textit{moyens du bord}. Cette section, présente quelques fonctionnalités fonctionnelles sur plaquette d'essais.
  			...
  			\begin{figure}
				...
				\textcolor{blue}{
					\put(-27,36){\framebox(22,22)}%servo
					\put(-11,2){\framebox(3,5)}%LED
					\put(-54,28){\framebox(3,3)}%LED
					\put(-60,8){\framebox(5,9)}%Fil AU
				}
				\textcolor{green}{
					\put(-35,10){\framebox(14,14)}%PWM
					\put(-54,8){\framebox(5,9)}%Fil bat
		...
		\end{figure}
  			...
  			\begin{table}
  			\begin{tabular}{c|c}
  			Composants réelement présent & composant remplacé\\
  			\hline
  			\pic & PWM \\ 
  			\dspic & Fil sous tension batterie\\
  			 Servo & Fil d'arrêt d'urgence   \\
  			 Module Bluetooth &\\ 
  			 LEDs &\\
  			  Arduino &\\
